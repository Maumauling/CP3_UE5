...
// Kernel zur Vektoraddition mit einem Faktor

__global__ void vec_add_gpu(double *p, double *r, double b, double *s, int nx, int ny)
{
   unsigned int ix = threadIdx.x + blockIdx.x * blockDim.x + 1;
   unsigned int iy = threadIdx.y + blockIdx.y * blockDim.y + 1;
   unsigned int idx = iy * (nx+2) + ix;
...
// Kernel zur Multiplikation zweier vektoren elementweise

__global__ void prod_gpu(double *v, double *a, double *b, int nx, int ny)
{
   unsigned int ix = threadIdx.x + blockIdx.x * blockDim.x + 1;
   unsigned int iy = threadIdx.y + blockIdx.y * blockDim.y + 1;
   unsigned int idx = iy * (nx+2) + ix;
